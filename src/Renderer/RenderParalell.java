...
public class RenderParalell {
    private Scene _scene;
    private ImageWriter _imageWriter;
    private final int MAX_CALC_COLOR_LEVEL = 3;

    //****************Constructor****************//
    public RenderParalell(Scene scene, ImageWriter imageWriter) {
        this._scene = scene;
    ...
    public void renderImage() throws InterruptedException {
        final ThreadPoolExecutor executor = new ThreadPoolExecutor(4, 8, 1, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>());
        for (int i = 1; i < _imageWriter.getNx(); i++) {
            for (int j = 1; j < _imageWriter.getNy(); j++) {
                final int ii = i;
                final int jj = j;
                executor.execute(() -> {
                    Ray ray = _scene.getCamera().constructRayThroughPixel(_imageWriter.getNx(), _imageWriter.getNy(), ii, jj, _scene.getDistance(), _imageWriter.getWidth(), _imageWriter.getHeight());
                    Map<Geometry, List<Point3D>> intersectionPoints = getSceneRayIntersections(ray);
                    if (intersectionPoints.isEmpty()||intersectionPoints == null) {
